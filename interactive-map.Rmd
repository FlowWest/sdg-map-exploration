...
output$data_table <- renderDT({
  ...
  filtered_data() |> 
    ...
    select(channel_id, min_daily_diff, avg_daily_diff, max_daily_diff) |> 
    filter(channel_id %in% c(71, 128, 205)) |> 
    mutate(
      min_diff = round(min_daily_diff, 1), 
      mean_diff = round(avg_daily_diff, 1),
      max_diff = round(max_daily_diff, 1)
    ) 
}, options = list(
  searching = FALSE,  
  lengthChange = FALSE,  
...
observe({
  ...
  palette <- colorNumeric(
    palette = "YlOrRd",
    domain = c(channels_merge$min_daily_diff, channels_merge$max_daily_diff)
  )

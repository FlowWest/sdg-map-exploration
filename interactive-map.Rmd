...
observe({
  ...
  leafletProxy("map") |>
    ...
    addPolylines(
      ...
      opacity = 1,
      label = ~name,
      color = ~palette(c(final_summary$min_diff, final_summary$max_diff)), #~palette(get(stat())),
      popup = ~sprintf(
    ...
    addLegend(
      ...
      pal = palette,
      values = c(final_summary$min_diff, final_summary$max_diff), #channels_merge[[stat()]],
      title = legend_title(),
      opacity = 1
    )
})
